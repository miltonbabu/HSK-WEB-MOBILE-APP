...
export default function WordActionCard({ word, onSpeak }: WordActionCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group flex items-center gap-2 sm:gap-2.5 px-2.5 sm:px-3 py-2 rounded-xl bg-white dark:bg-white/5 border border-ink-100 dark:border-white/10 hover:border-red-300 dark:hover:border-red-500/50 hover:shadow-sm transition-all"
    >
      <button
        onClick={() => onSpeak(word.chinese)}
        className="shrink-0 w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors"
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-1.5 sm:gap-2 flex-wrap">
          ...
          <span className="px-1 py-0.5 rounded text-[8px] sm:text-[9px] font-bold text-white bg-red-500/90 shrink-0">
            HSK {word.hsk_level}
          </span>
        </div>
      ...
      <div className="flex items-center gap-0.5 sm:gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
        <Link
          ...
          className="shrink-0 w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center text-ink-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors"
