...
import { Timer, CheckCircle, XCircle, Share2, RotateCcw, Trophy, Clock, Target, Zap, ChevronRight } from 'lucide-react-native';
...
export default function TimedQuizMode() {
  ...
  const endGame = async () => {
    ...
    try {
      ...
      const accuracy = finalResults.length > 0 ? Math.round((finalCorrect / finalResults.length) * 100) : 0;
  ...
  if (words.length < 4) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-brand-50 dark:bg-ink-950 px-6" edges={['bottom']}>
        <Timer size={64} color="#9ca3af" />
        <Text className="text-xl font-semibold text-ink-900 dark:text-white mt-4">Not Enough Words</Text>
      ...
      <SafeAreaView className="flex-1 bg-brand-50 dark:bg-ink-950" edges={['bottom']}>
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
          <View className="items-center mb-6">
            ...
            <Text className="text-2xl font-bold text-ink-900 dark:text-white">Timed Quiz</Text>
            <Text className="text-ink-500 dark:text-ink-400 mt-2 text-center">
          ...
          <View className="bg-white dark:bg-ink-900 rounded-2xl p-5 mb-4 shadow-sm">
            ...
            <View className="flex-row flex-wrap justify-center gap-2">
              {([1, 2, 3, 4] as HSKLevel[]).map((level) => (
                <Pressable
                  key={level}
                  onPress={() => { setSelectedLevel(level); triggerHaptic('light'); }}
          ...
          <View className="bg-white dark:bg-ink-900 rounded-2xl p-5 mb-4 shadow-sm">
            {/* Quiz Mode */}
            <View className="mb-5">
              <Text className="text-sm font-semibold text-ink-700 dark:text-ink-300 mb-3">Quiz Mode</Text>
              <View className="flex-row justify-center gap-2">
                {QUIZ_MODE_OPTIONS.map((opt) => (
                  <Pressable
                    key={opt.id}
                    onPress={() => { setQuizMode(opt.id); triggerHaptic('light'); }}
                    className={`px-4 py-2 rounded-xl ${
                      quizMode === opt.id
                        ? 'bg-brand-500'
                        : 'bg-ink-100 dark:bg-ink-800'
            ...
            {/* Question Count */}
            <View className="mb-5">
              <Text className="text-sm font-semibold text-ink-700 dark:text-ink-300 mb-3">Number of Questions</Text>
              <View className="flex-row flex-wrap justify-center gap-2">
                {QUESTION_COUNTS.map((count) => (
                  <Pressable
                    key={count}
                    onPress={() => { setQuestionCount(count); triggerHaptic('light'); }}
                    className={`px-4 py-2 rounded-xl ${
                      questionCount === count
                        ? 'bg-brand-500'
                        : 'bg-ink-100 dark:bg-ink-800'
            ...
            {/* Timer */}
            <View className="mb-5">
              <Text className="text-sm font-semibold text-ink-700 dark:text-ink-300 mb-3">Time per Question</Text>
              <View className="flex-row justify-center gap-2">
                {TIMER_OPTIONS.map((seconds) => (
                  <Pressable
                    key={seconds}
                    onPress={() => { setTimerSeconds(seconds); triggerHaptic('light'); }}
                    className={`px-4 py-2 rounded-xl ${
            ...
            <View className="flex-row items-center justify-center gap-4 mb-4">
              <View className="flex-row items-center gap-1.5">
                ...
                <Text className="text-sm text-ink-500 dark:text-ink-400">{questionCount} questions</Text>
              </View>
              <View className="flex-row items-center gap-1.5">
                <Clock size={16} color="#6b7280" />
                <Text className="text-sm text-ink-500 dark:text-ink-400">{timerSeconds}s each</Text>
              </View>
            </View>
      ...
      <SafeAreaView className="flex-1 bg-brand-50 dark:bg-ink-950" edges={['bottom']}>
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
          ...
          >
            ...
            <Text className="text-2xl font-bold text-ink-900 dark:text-white mb-4">Quiz Complete!</Text>
            ...
            <View className="flex-row justify-center gap-6 mb-4">
              <View className="items-center">
                <Text className="text-2xl font-bold text-brand-500">{score}</Text>
                <Text className="text-xs text-ink-500 dark:text-ink-400">Score</Text>
              </View>
              <View className="items-center">
                <Text className="text-2xl font-bold text-ink-900 dark:text-white">{correctCount}/{questionCount}</Text>
                <Text className="text-xs text-ink-500 dark:text-ink-400">Correct</Text>
              </View>
              <View className="items-center">
                <Text className="text-2xl font-bold text-ink-900 dark:text-white">{accuracy}%</Text>
                <Text className="text-xs text-ink-500 dark:text-ink-400">Accuracy</Text>
              </View>
            </View>
            ...
            <View className="flex-row items-center justify-center gap-4">
              <View className="flex-row items-center gap-1.5">
                <Clock size={16} color="#6b7280" />
                <Text className="text-sm text-ink-500 dark:text-ink-400">Avg {avgTime}s</Text>
              </View>
              <View className="flex-row items-center gap-1.5">
                <Zap size={16} color="#6b7280" />
                <Text className="text-sm text-ink-500 dark:text-ink-400">HSK {selectedLevel}</Text>
              </View>
            </View>
          </View>
          ...
          <View className="bg-white dark:bg-ink-900 rounded-2xl p-4 mb-4 shadow-sm">
            <Text className="text-sm font-semibold text-ink-700 dark:text-ink-300 mb-3">Question Review</Text>
            <View className="max-h-60">
              {results.map((r, i) => (
                ...
                >
                  ...
                  <Text className="text-sm font-medium text-ink-900 dark:text-white">{r.word.chinese}</Text>
                  <Text className="text-xs text-ink-500 dark:text-ink-400 flex-1" numberOfLines={1}>{r.word.english}</Text>
                  <Text className="text-xs text-ink-400 dark:text-ink-500">{r.timeTaken.toFixed(1)}s</Text>
                </View>
              ))}
            </View>
          </View>
          ...
          <Pressable
            onPress={() => { setPhase('setup'); setResults([]); triggerHaptic('light'); }}
            className="flex-row items-center justify-center gap-2 py-3 rounded-2xl active:opacity-60"
          >
            <RotateCcw size={16} color="#6b7280" />
            <Text className="text-sm font-semibold text-ink-500 dark:text-ink-400">New Quiz</Text>
          </Pressable>
        </ScrollView>
      </SafeAreaView>
    );
      ...
      >
        ...
        <View className="flex-row items-center justify-between mb-4">
          <View>
            ...
            <Text className="text-2xl font-bold text-brand-500">{score} pts</Text>
          </View>
          <View className="flex-row items-center gap-3">
            <Text className="text-sm text-ink-500 dark:text-ink-400">{correctCount} ✓</Text>
        ...
        >
          {quizMode === 'zh-en' && (
            <>
              <Text className="text-center text-sm text-ink-400 dark:text-ink-500 mb-2">What does this mean?</Text>
              <Pressable onPress={() => currentQuestion && speak(currentQuestion.chinese).catch(() => {})}>
          ...
          {quizMode === 'en-zh' && (
            <>
              <Text className="text-center text-sm text-ink-400 dark:text-ink-500 mb-2">Which character matches this?</Text>
          ...
          {quizMode === 'py-zh' && (
            <>
              <Text className="text-center text-sm text-ink-400 dark:text-ink-500 mb-2">Which character sounds like this?</Text>
          ...
          {options.map((option, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  <View className={`w-6 h-6 rounded-lg items-center justify-center ${badgeBg}`}>
                    <Text className={`text-xs font-bold ${badgeText}`}>
                      {String.fromCharCode(65 + index)}
                    </Text>
                  </View>
                  <Text className={`flex-1 font-medium text-sm pt-0.5 ${textClass} ${isChineseOption ? 'text-lg' : ''}`} numberOfLines={2}>
                    {option.text}
                  </Text>
                </Pressable>
              </MotiView>
